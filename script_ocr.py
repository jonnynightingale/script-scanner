...
def load_character_mapping(character_tsv_file):
...
def remove_color(image, threshold=30):
    ...
    non_grey_mask = (diff_rg > threshold) | (
        diff_rb > threshold) | (diff_gb > threshold)
...
def crop_to_character_names(script_image):
    ...
    if width < 1500:
        raise RuntimeError(
            f"Image is narrower than expected. width = {width}.")
...
def map_scanned_character_names_to_json_equivalent(character_mapping, names):
    ...
    for name in names:
        ...
        def dist(x): return Levenshtein.distance(name, x)
        (closest_match, distance) = min(((key, dist(key))
                                         for key in character_mapping.keys()), key=lambda x: x[1])
        if distance <= 3:
        ...
        raise RuntimeError(
            f"Failed to find a match for character name: {name}")

    return json_characters


def extract_character_names(character_mapping, script_image):
    ...
    character_names_image = cv2.cvtColor(
        character_names_image, cv2.COLOR_BGR2RGB)
    ...
    try:
        ...
        raise RuntimeError(
            "Image processing took too long when scanning character names") from timeout_error
    ...
    return map_scanned_character_names_to_json_equivalent(character_mapping, character_names)
...
def extract_script_meta_data(script_image):
    ...
    try:
        ...
        raise RuntimeError(
            "Image processing took too long when scanning script metadata") from timeout_error
...
def script_image_to_json(character_mapping, input_image):
    ...
    character_names = extract_character_names(
        character_mapping, processed_script_image)
